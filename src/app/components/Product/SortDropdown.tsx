...
export default function SortDropdown({ filters, setFilters }: any) {
  ...
  return (
    <div ref={ref} className="relative w-[180px] lg:w-[200px]">
      {/* Trigger button */}
      <button
        type="button"
        ...
        className={`
          ...
          px-3 py-[12px] pr-9
          text-[13px] font-normal tracking-wide text-left rounded-sm
          border hover:border-[#F15939] bg-white cursor-pointer
          transition-all
          ${open ? " " : ""}
        `}
      >
        ...
        <ul className="absolute z-50 w-full bg-white border border-gray-500 border-t-0 rounded-b-md shadow-sm ">
          {sortOptions.map((opt) => (
            <li
              ...
              className={`
          px-3 py-[8px] text-[13px] font-normal tracking-wide
