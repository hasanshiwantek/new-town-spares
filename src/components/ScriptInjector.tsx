...
export default function ScriptInjector() {
  ...
  const activeScripts =
    scripts?.filter(
      (s: any) => s.location === "allPages" || s.location === currentLocation,
    ) || [];

  const headerScripts = activeScripts.filter(
    (s: any) => s.placement === "header",
  );
  const footerScripts = activeScripts.filter(
    (s: any) => s.placement === "footer",
  );
