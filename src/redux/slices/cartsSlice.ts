...
export const clearAllCart = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.delete("/web/cart/clear-all");
...
export const addBySku = createAsyncThunk(
  "cart/addBySku",
  async (data: { sku: string; quantity: number }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post("/web/cart/add-by-sku", data, {
        headers: {
          storeId: 10,
        },
      });
...
const cartsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addBySku.pending, (state) => {
        state.loading = true;
      })
      .addCase(addBySku.fulfilled, (state, action) => {
        state.loading = false;
      })
      .addCase(addBySku.rejected, (state, action: any) => {
        state.loading = false;
        state.error = action.error.message || "Failed update cart";
